...
#'  was written are installed. Various warning messages are thrown when these
#'  conditions are violated to make the user aware. This script is called at
#'  the start of many function scripts to provide the relevant warnings for
#'  users.
...
warning_messages <- function(x) {
  ...
  packs.df <- data.frame(
    installed.packages = sort(pack.ver),
