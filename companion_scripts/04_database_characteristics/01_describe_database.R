...
# need to decide on some higher classification scheme so that
# we can quantify the coverage... How detailed should it be?
...
# select the relevant columns for describing the taxonomic distribution
# in the database
names(tax.dat)
tax.dat <-
  tax.dat %>%
  dplyr::select(
    group1, group2, db_taxon, db_higher_rank_source,
    id, scientificName, family, db_taxon_higher_rank,
    db_taxon_higher
  )
View(tax.dat)
