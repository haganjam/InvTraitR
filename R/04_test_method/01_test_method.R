...
source(here::here("R/03_use_database/01_search_database_ver2.R"))
...
assertthat::assert_that(
  assertthat::see_if(all(y == TRUE | is.na(y))),
  msg = error_string
)

assertthat::assert_that(
  assertthat::see_if(all(apply(x, 1, function(x) sum(is.na(x))) == test1.na.output)),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(names(x) == c(
    "latitude",
    "longitude",
    "site",
    "habitat_id",
...
assertthat::assert_that(
  assertthat::see_if(all(x[["site"]] == df.test2[["site"]])),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(x[["site"]] == c(1, 2, 3, 4, 5, 6, 9, 7, 8))),
  msg = error_string
)

assertthat::assert_that(
  assertthat::see_if(all(x[["sex"]] == c(
    "male",
    "female",
    "female",
    "male",
...
assertthat::assert_that(
  assertthat::see_if(all(x$db == "special")),
  msg = error_string
)

assertthat::assert_that(
  assertthat::see_if(all(x$clean_taxon_name == c("Turbellaria", "Nematoda"))),
  msg = error_string
)
...
# test if the output is correct
assertthat::assert_that(
  assertthat::see_if(all(x$db == "gbif" | is.na(x$db))),
  msg = error_string
)
...
t1 <-
  lapply(z1, function(input) {
    all(names(input) == c(
      ...
      "db_taxon_higher", "habitat_id", "realm", "major_habitat_type",
      "ecoregion", "db.scientificName", "trait_out", "id", "tax_distance"
    ))
  })

assertthat::assert_that(
  assertthat::see_if(all(unlist(t1))),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(is.na(t2) | (t2 == TRUE))),
  msg = error_string
)
...
df.test1 <-
  data.frame(
    taxon_name = c(
      "Gammarus", "Gammarus", "Gammarus", "Daphnia", "Triops granitica",
      "Triops", "Simocephalus vetulus", "Simocephalus vetulus", "Turbellaria",
      "Oligochaeta", "Oligochaeta", "Daphnia gessneri"
    ),
    Life_stage = c(
...
assertthat::assert_that(
  assertthat::see_if(all(x[["taxon_name"]] == df.test1[["taxon_name"]])),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(t1 | is.na(t1))),
  msg = error_string
)

# if the equation is NA then the dry_biomass_mg should be NA
assertthat::assert_that(
  assertthat::see_if(all(is.na(x[["equation"]]) == is.na(x[["dry_biomass_mg"]]))),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(is.na(x[["body_size_mm"]][t2]) == is.na(x[["dry_biomass_mg"]][t2]))),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(is.na(c(x[["dry_biomass_mg"]][t3], x[["equation"]][t3])))),
  msg = error_string
)

# check if the outputted taxonomic distances are less than the max tax distance
assertthat::assert_that(
  assertthat::see_if(all(x[["tax_distance"]][!is.na(x[["tax_distance"]])] < 3)),
  msg = error_string
)
...
assertthat::assert_that(
  assertthat::see_if(all(df.test3[["taxon_name"]] == x[["taxon_name"]])),
  msg = error_string
)
