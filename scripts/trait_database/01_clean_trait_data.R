...
# add a median column
axell.lw$TL_middle <- ifelse(is.na(axell.lw$TL_max), axell.lw$TL_min, NA)

# remove the duplicate minima 
axell.lw$TL_min <- ifelse(!is.na(axell.lw$TL_middle), NA, axell.lw$TL_min)
...
# remove the duplicate minima 
axell.lw$DW_min <- ifelse(!is.na(axell.lw$DW_middle), NA, axell.lw$DW_min)

## Reorder
# reorganise the columns
axell.lw <- 
  axell.lw %>%
  select(Genus, Species, LifeStage, TL_reference = `TL (reference)`,
